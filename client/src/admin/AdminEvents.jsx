...
import { adminGetEvents, adminApproveEvent, adminDeleteEvent, adminAddEvent } from '../api';
...
const EMPTY_FORM = { title: '', description: '', date: '', time: '', format: 'Online', location: '', host: '' };
...
export default function AdminEvents() {
  ...
  const inputStyle = {
    ...
    fontSize: 13, background: 'var(--surface-muted)',
    ...
    <div>
      ...
      {showForm && (
        <div style={{
          ...
          <h2 style={{ fontSize: 15, fontWeight: 700, color: 'var(--text-strong)', marginBottom: 18 }}>
            New event (auto-approved)
          </h2>
          <form onSubmit={handleAddEvent}>
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: 14, marginBottom: 14 }}>
              <div>
                <label style={{ display: 'block', fontSize: 12, fontWeight: 500, color: 'var(--text-muted)', marginBottom: 5 }}>Title *</label>
                <input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} style={inputStyle} required />
              </div>
              <div>
                <label style={{ display: 'block', fontSize: 12, fontWeight: 500, color: 'var(--text-muted)', marginBottom: 5 }}>Host</label>
                <input value={form.host} onChange={e => setForm(f => ({ ...f, host: e.target.value }))} style={inputStyle} />
              </div>
              <div>
                <label style={{ display: 'block', fontSize: 12, fontWeight: 500, color: 'var(--text-muted)', marginBottom: 5 }}>Date *</label>
                <input type="date" value={form.date} onChange={e => setForm(f => ({ ...f, date: e.target.value }))} style={inputStyle} required />
              </div>
              <div>
                <label style={{ display: 'block', fontSize: 12, fontWeight: 500, color: 'var(--text-muted)', marginBottom: 5 }}>Time</label>
                <input type="time" value={form.time} onChange={e => setForm(f => ({ ...f, time: e.target.value }))} style={inputStyle} />
              </div>
              <div>
                <label style={{ display: 'block', fontSize: 12, fontWeight: 500, color: 'var(--text-muted)', marginBottom: 5 }}>Format</label>
            ...
            <button type="submit" disabled={submitting} style={{
              ...
              color: 'var(--brand-contrast)', border: 'none',
      ...
      {pending.length > 0 && (
        <div style={{ marginBottom: 28 }}>
          <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-subtle)', textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 10 }}>
              ...
              <tbody>
                {pending.map(event => (
                  <tr key={event._id} style={{ borderBottom: '1px solid var(--border)' }}>
                    <Td>
                      <div style={{ fontWeight: 600, color: 'var(--text-strong)', fontSize: 13 }}>{event.title}</div>
                      {event.description && (
                    ...
                    <Td>
                      <div style={{ display: 'flex', gap: 8 }}>
                        ...
                        >
                          {actionLoading === event._id + '-d' ? '...' : 'Reject'}
                        </button>
                      </div>
                    </Td>
                  </tr>
              ...
              <tbody>
                {approved.map(event => (
                  <tr key={event._id} style={{ borderBottom: '1px solid var(--border)' }}>
                    <Td>
                      <div style={{ fontWeight: 600, color: 'var(--text-strong)', fontSize: 13 }}>{event.title}</div>
                      {event.host && <div style={{ fontSize: 12, color: 'var(--text-subtle)', marginTop: 2 }}>by {event.host}</div>}
                    </Td>
                    <Td>{fmtDate(event.date)}</Td>
                    <Td>
                      <span style={{
